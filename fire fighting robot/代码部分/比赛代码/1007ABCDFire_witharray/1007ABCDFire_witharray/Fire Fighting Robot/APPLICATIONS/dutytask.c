...
void Duty_10ms(void)
{
	...
	//DatatransferTask();
  //judge_fire();
	//MotorLeft(63);	left63 right80 can go straight
	//MotorRight(80);
	//delay_ms(4000);
...
void Duty_50ms(void)
{
	static u8 FirePosition[4]={0,0,1,1};  //标号是火的标号(火的标号是按照车轨迹的先后） 值0表示无火 值1表示有火
	static u8 candle=2;
	...
	if(left_encoder_count>=120||right_encoder_count>=120){			//yuan 110
			turn_left_withdelay();
			along_flag=0;
			FLAG++;
			CLR=1;
